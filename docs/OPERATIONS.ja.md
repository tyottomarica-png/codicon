...
Codicon は、Xbox コントローラから手を離さず **Claude Code と Codex** の指示・モデル・推論量・速度・承認を操作するためのデスクトップアプリです。Linux と macOS で同じ標準 Gamepad マッピングを使用します。
...
### Power Ring を他のアプリの上で使う

Claude Code や Codex のウィンドウを前面にしたまま **LB を押すと、画面中央に同じデザインの円形セレクタがオーバーレイ表示**されます。左スティックでモデル、右スティックで推論量を選び、LB を離して確定。クリックを奪わない表示専用ウィンドウなので、作業中のアプリの操作を妨げません。
